...
def triangular_quad_points(dim, nquadpoints, dtype):
    if dim == 2:
        if nquadpoints == 1:
            ...
            iso_coords = np.array([[1/6, 1/6], [2/3, 1/6], [1/6, 2/3]], dtype=dtype)
            weights = np.array([1/6, 1/6, 1/6], dtype=dtype)
            return iso_coords, weights

class CST:
    def __init__(self):
        iso_coords, weights = triangular_quad_points(2, 1, np.float64)
        self.iso_coords = iso_coords
        self.weights = weights
    
    def jacobian(self, xe):
        # not generally constant, could depend on iso_coord (xi)
        x1, x2, x3 = xe
    ...
    def B_operator(self, xe):
        ...
        B = np.zeros((3,2*dNdx.shape[0])) # not ideal to hard-code dimensions, but will never change for 2D elements
        for i in range(dNdx.shape[0]):
            dNidx = dNdx[i][0]
            dNidy = dNdx[i][1]
            B[0, 2*i] = dNidx
